...
class StructureFocusManager extends component_1.StatefulPluginComponent {
    ...
    tryGetRange(loci) {
        return (0, selection_1.getLociRange)(this.referenceLoci, loci);
    }
    setFromLoci(anyLoci) {
    ...
    addFromLoci(anyLoci) {
        ...
        const refLoci = loci_1.Loci.normalize(anyLoci);
        this.referenceLoci = structure_1.StructureElement.Loci.is(refLoci) ? refLoci : undefined;
    }
    toggleFromLoci(anyLoci) {
        ...
        const refLoci = loci_1.Loci.normalize(anyLoci);
        this.referenceLoci = structure_1.StructureElement.Loci.is(refLoci) && kind !== 'subtract' ? refLoci : undefined;
    }
...
function toggleLoci(currentLoci, newLoci) {
    if (currentLoci && structure_1.StructureElement.Loci.is(newLoci) && newLoci.structure === currentLoci.structure) {
        if (structure_1.StructureElement.Loci.isSubset(currentLoci, newLoci)) {
            return { kind: 'subtract', loci: structure_1.StructureElement.Loci.subtract(currentLoci, newLoci) };
        }
        else {
            return { kind: 'add', loci: structure_1.StructureElement.Loci.union(newLoci, currentLoci) };
        }
    }
    else {
