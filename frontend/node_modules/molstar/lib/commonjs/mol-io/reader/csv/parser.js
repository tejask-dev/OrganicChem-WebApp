...
var CsvTokenType;
(function (CsvTokenType) {
    CsvTokenType[CsvTokenType["Value"] = 0] = "Value";
    CsvTokenType[CsvTokenType["Comment"] = 1] = "Comment";
    CsvTokenType[CsvTokenType["End"] = 2] = "End";
})(CsvTokenType || (CsvTokenType = {}));
...
function readRecordsChunk(chunkSize, state) {
    if (state.tokenType === CsvTokenType.End)
        return 0;
    let counter = 0;
    let newRecord;
...
function readRecordsChunks(state) {
    ...
    if (newRecord)
        ++state.recordCount;
    return (0, mol_task_1.chunkedSubtask)(state.runtimeCtx, 100000, state, readRecordsChunk, (ctx, state) => ctx.update({ message: 'Parsing...', current: state.tokenizer.position, max: state.data.length }));
}
...
function parseCsv(data, opts) {
    ...
    return mol_task_1.Task.create('Parse CSV', async (ctx) => {
