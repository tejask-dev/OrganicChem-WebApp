...
(function (ValidationReport) {
    ...
    function isApplicable(model) {
        return !!model && model_1.Model.hasPdbId(model) && !model_1.Model.isIntegrative(model);
    }
    ...
    async function fetch(ctx, model, props) {
        const url = assets_1.Asset.getUrlAsset(ctx.assetManager, getEntryUrl(model.entryId, props.baseUrl));
    ...
    ValidationReport.symbols = {
        hasClash: compiler_1.QuerySymbolRuntime.Dynamic((0, symbol_1.CustomPropSymbol)('rcsb', 'validation-report.has-clash', type_1.Type.Bool), ctx => {
            const { unit, element } = ctx.element;
            if (!structure_1.Unit.isAtomic(unit))
                return 0;
            const validationReport = exports.ValidationReportProvider.get(unit.model).value;
            return validationReport && validationReport.clashes.getVertexEdgeCount(element) > 0;
        }),
        issueCount: compiler_1.QuerySymbolRuntime.Dynamic((0, symbol_1.CustomPropSymbol)('rcsb', 'validation-report.issue-count', type_1.Type.Num), ctx => {
            var _a;
            const { unit, element } = ctx.element;
            if (!structure_1.Unit.isAtomic(unit))
                return 0;
...
exports.ValidationReportProvider = custom_model_property_1.CustomModelProperty.createProvider({
    ...
    getParams: (data) => exports.ValidationReportParams,
    isApplicable: (data) => ValidationReport.isApplicable(data),
    obtain: async (ctx, data, props) => {
        const p = { ...param_definition_1.ParamDefinition.getDefaultValues(exports.ValidationReportParams), ...props };
...
function createInterUnitClashes(structure, clashes) {
    ...
    structure_1.Structure.eachUnitPair(structure, (unitA, unitB) => {
        ...
        for (let i = 0, il = clashes.edgeCount * 2; i < il; ++i) {
            // TODO create lookup
            const indexA = int_1.SortedArray.indexOf(elementsA, a[i]);
            const indexB = int_1.SortedArray.indexOf(elementsB, b[i]);
...
function createIntraUnitClashes(unit, clashes) {
    ...
    for (let i = 0, il = edgeCount * 2; i < il; ++i) {
        // TODO create lookup
        const indexA = int_1.SortedArray.indexOf(elements, a[i]);
        const indexB = int_1.SortedArray.indexOf(elements, b[i]);
        if (indexA !== -1 && indexB !== -1) {
    ...
    const builder = new graph_1.IntAdjacencyGraph.EdgeBuilder(elements.length, aIndices, bIndices);
...
function createClashes(structure, clashes) {
    const intraUnit = int_1.IntMap.Mutable();
...
exports.ClashesProvider = custom_structure_property_1.CustomStructureProperty.createProvider({
    ...
    obtain: async (ctx, data) => {
        await exports.ValidationReportProvider.attach(ctx, data.models[0]);
        const validationReport = exports.ValidationReportProvider.get(data.models[0]).value;
        return {
...
function ClashesBuilder(elementsCount) {
    ...
    return {
        ...
        get() {
            const builder = new graph_1.IntAdjacencyGraph.EdgeBuilder(elementsCount, aIndices, bIndices);
            const id = new Int32Array(builder.slotCount);
            const magnitude = new Float32Array(builder.slotCount);
            const distance = new Float32Array(builder.slotCount);
            for (let i = 0, _i = builder.edgeCount; i < _i; i++) {
