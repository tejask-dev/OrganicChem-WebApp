...
function writeValue(builder, data, key, f, floatPrecision, index) {
    ...
    const p = kind ? kind(key, data) : db_1.Column.ValueKinds.Present;
    if (p !== db_1.Column.ValueKinds.Present) {
        if (p === db_1.Column.ValueKinds.NotPresent)
        ...
        else if (t === encoder_1.Field.Type.Int) {
...
function getFloatPrecisions(categoryName, fields, formatter) {
    ...
    for (const f of fields) {
        ...
        if (format && typeof format.digitCount !== 'undefined')
            ret[ret.length] = f.type === encoder_1.Field.Type.Float ? Math.pow(10, Math.max(0, Math.min(format.digitCount, 15))) : 0;
        else
            ret[ret.length] = f.type === encoder_1.Field.Type.Float ? Math.pow(10, (0, util_1.getFieldDigitCount)(f)) : 0;
    }
    return ret;
}
