    ...
    update: ({ b, oldParams, newParams, cache, dependencies }) => {
        ...
        for (const sel of newParams.selections) {
            ...
            if (cacheEntries.has(sel.key)) {
                ...
                if (prev && source === entry.source && sel.bundle.hash === entry.source.hashCode && structure_1.StructureElement.Bundle.areEqual(sel.bundle, prev)) {
                    const loci = current.get(sel.key);
                    if (loci.groupId !== sel.groupId) {
                        loci.groupId = sel.groupId;
                        changed = true;
...
exports.MultiStructureSelectionFromBundle = MultiStructureSelectionFromBundle;
