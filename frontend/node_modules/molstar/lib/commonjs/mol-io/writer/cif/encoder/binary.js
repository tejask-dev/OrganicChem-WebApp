...
const encode_1 = require("../../../common/msgpack/encode");
const binary_cif_1 = require("../../../common/binary-cif");
const encoder_1 = require("../encoder");
const util_1 = require("./util");
const classifier_1 = require("../../../common/binary-cif/classifier");
class BinaryEncoder {
    setFilter(filter) {
    ...
    encode() {
        if (this.encodedData)
            return;
        this.encodedData = (0, encode_1.encodeMsgPack)(this.data);
        this.data = null;
...
function getArrayCtor(field, format) {
    ...
    if (field.type === encoder_1.Field.Type.Str)
        return Array;
    if (field.type === encoder_1.Field.Type.Int)
        return Int32Array;
    return Float64Array;
}
function getDefaultEncoder(type) {
    if (type === encoder_1.Field.Type.Str)
        return binary_cif_1.ArrayEncoder.by(binary_cif_1.ArrayEncoding.stringArray);
    return binary_cif_1.ArrayEncoder.by(binary_cif_1.ArrayEncoding.byteArray);
}
...
function classify(type, data) {
    if (type === encoder_1.Field.Type.Str)
        return binary_cif_1.ArrayEncoder.by(binary_cif_1.ArrayEncoding.stringArray);
    if (type === encoder_1.Field.Type.Int)
        return (0, classifier_1.classifyIntArray)(data);
    return (0, classifier_1.classifyFloatArray)(data);
}
function encodeField(categoryName, field, data, totalCount, format, encoderProvider, autoClassify) {
    ...
    if (field.type === encoder_1.Field.Type.Str) {
        ...
        encoder = binary_cif_1.ArrayEncoder.by(binary_cif_1.ArrayEncoding.stringArray);
    }
    else {
    ...
    let maskData = void 0;
    if (!allPresent) {
        const maskRLE = binary_cif_1.ArrayEncoder.by(binary_cif_1.ArrayEncoding.runLength).and(binary_cif_1.ArrayEncoding.byteArray).encode(mask);
        if (maskRLE.data.length < mask.length) {
            maskData = maskRLE;
        }
        else {
            maskData = binary_cif_1.ArrayEncoder.by(binary_cif_1.ArrayEncoding.byteArray).encode(mask);
        }
    }
    return {
        name: field.name,
...
function getFieldData(field, arrayCtor, totalCount, data) {
    const isStr = field.type === encoder_1.Field.Type.Str;
    ...
    for (let _d = 0; _d < data.length; _d++) {
        ...
        while (keys.hasNext) {
            const key = keys.move();
            const p = valueKind ? valueKind(key, d) : db_1.Column.ValueKinds.Present;
            if (p !== db_1.Column.ValueKinds.Present) {
                mask[offset] = p;
                if (isStr)
                    array[offset] = '';
                allPresent = false;
                ...
                if (typeof value === 'string' && !value) {
                    mask[offset] = db_1.Column.ValueKinds.NotPresent;
                    allPresent = false;
                }
                else {
                    mask[offset] = db_1.Column.ValueKinds.Present;
                }
                array[offset] = value;
            }
            offset++;
