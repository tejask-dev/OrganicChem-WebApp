...
function addSrc(seqToSrcByModelEntity, srcKeySerialMap, modelIndex, model, entity_src, scientific_name, plasmid_name, gene_src_gene) {
    ...
    for (let j = 0, jl = entity_src._rowCount; j < jl; ++j) {
        ...
        const beg = pdbx_beg_seq_num.valueKind(j) === db_1.Column.ValueKinds.Present ? pdbx_beg_seq_num.value(j) : 1;
        const end = pdbx_end_seq_num.valueKind(j) === db_1.Column.ValueKinds.Present ? pdbx_end_seq_num.value(j) : seqToSrc.length;
