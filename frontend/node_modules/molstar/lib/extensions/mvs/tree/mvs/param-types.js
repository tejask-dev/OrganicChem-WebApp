...
import { float, int, list, literal, str, tuple, union } from '../generic/field-schema';
...
export const HexColorT = new iots.Type('HexColor', ((value) => typeof value === 'string'), (value, ctx) => HexColor.is(value) ? { _tag: 'Right', right: value } : { _tag: 'Left', left: [{ value: value, context: ctx, message: `"${value}" is not a valid hex color string` }] }, value => value);
/** `color` parameter values for `color` node in MVS tree */
export const ColorNameT = new iots.Type('ColorName', ((value) => typeof value === 'string'), (value, ctx) => ColorName.is(value) ? { _tag: 'Right', right: value } : { _tag: 'Left', left: [{ value: value, context: ctx, message: `"${value}" is not a valid hex color string` }] }, value => value);
/** `color` parameter values for `color` node in MVS tree */
export const ColorNamesT = literal(...Object.keys(ColorNames));
/** `color` parameter values for `color` node in MVS tree */
