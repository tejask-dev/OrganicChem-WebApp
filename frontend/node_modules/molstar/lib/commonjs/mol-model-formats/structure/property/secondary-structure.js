...
(function (ModelSecondaryStructure) {
    ...
    function fromStruct(conf, sheetRange, hierarchy) {
        ...
        return (0, secondary_structure_1.SecondaryStructure)(secStruct.type, secStruct.key, secStruct.elements, getIndex);
    }
    ModelSecondaryStructure.fromStruct = fromStruct;
})(ModelSecondaryStructure || (exports.ModelSecondaryStructure = ModelSecondaryStructure = {}));
function getCoordinateType(conf, sheetRange) {
    if (conf._rowCount > 0) {
        if (conf.beg_label_seq_id.valueKind(0) !== db_1.Column.ValueKinds.Present || conf.end_label_seq_id.valueKind(0) !== db_1.Column.ValueKinds.Present)
            return 'auth';
    }
    else if (sheetRange) {
        if (sheetRange.beg_label_seq_id.valueKind(0) !== db_1.Column.ValueKinds.Present || sheetRange.end_label_seq_id.valueKind(0) !== db_1.Column.ValueKinds.Present)
            return 'auth';
    }
    return 'label';
}
function addHelices(cat, coordinates, map, elements) {
    ...
    for (let i = 0, _i = cat._rowCount; i < _i; i++) {
        const type = types_1.SecondaryStructureType.create(pdbx_PDB_helix_class.valueKind(i) === db_1.Column.ValueKinds.Present
            ? types_1.SecondaryStructureType.SecondaryStructurePdb[pdbx_PDB_helix_class.value(i)]
            : conf_type_id.valueKind(i) === db_1.Column.ValueKinds.Present
                ? types_1.SecondaryStructureType.SecondaryStructureMmcif[conf_type_id.value(i)]
                : types_1.SecondaryStructureType.Flag.NA);
        const element = {
            kind: 'helix',
            flags: type,
            type_id: conf_type_id.valueKind(i) === db_1.Column.ValueKinds.Present ? conf_type_id.value(i) : 'helx_p',
            helix_class: pdbx_PDB_helix_class.value(i),
            details: details.valueKind(i) === db_1.Column.ValueKinds.Present ? details.value(i) : void 0
        };
        const entry = {
...
function addSheets(cat, coordinates, map, sheetCount, elements) {
    ...
    for (let i = 0, _i = cat._rowCount; i < _i; i++) {
        ...
        const type = types_1.SecondaryStructureType.create(types_1.SecondaryStructureType.Flag.Beta | types_1.SecondaryStructureType.Flag.BetaSheet);
        const element = {
            kind: 'sheet',
            flags: type,
            sheet_id: id,
...
function assignSecondaryStructureRanges(hierarchy, coordinates, map, data) {
    ...
    for (let cI = 0; cI < chainCount; cI++) {
        const resStart = atomic_1.AtomicHierarchy.chainStartResidueIndex(hierarchy, cI), resEnd = atomic_1.AtomicHierarchy.chainEndResidueIndexExcl(hierarchy, cI);
