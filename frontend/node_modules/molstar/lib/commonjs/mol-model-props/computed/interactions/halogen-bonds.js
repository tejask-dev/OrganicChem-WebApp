...
const halBondElements = [types_1.Elements.CL, types_1.Elements.BR, types_1.Elements.I, types_1.Elements.AT];
/**
...
function addUnitHalogenDonors(structure, unit, builder) {
    ...
    for (let i = 0, il = elements.length; i < il; ++i) {
        ...
        if (halBondElements.includes(element)) {
            builder.add(common_1.FeatureType.HalogenDonor, common_1.FeatureGroup.None, x[elements[i]], y[elements[i]], z[elements[i]], i);
        }
    }
}
const X = [types_1.Elements.N, types_1.Elements.O, types_1.Elements.S];
const Y = [types_1.Elements.C, types_1.Elements.N, types_1.Elements.P, types_1.Elements.S];
/**
...
function addUnitHalogenAcceptors(structure, unit, builder) {
    ...
    for (let i = 0, il = elements.length; i < il; ++i) {
        ...
        if (X.includes(element)) {
            ...
            if (flag) {
                builder.add(common_1.FeatureType.HalogenAcceptor, common_1.FeatureGroup.None, x[elements[i]], y[elements[i]], z[elements[i]], i);
            }
        }
    }
}
function isHalogenBond(ti, tj) {
    return ((ti === common_1.FeatureType.HalogenAcceptor && tj === common_1.FeatureType.HalogenDonor) ||
        (ti === common_1.FeatureType.HalogenDonor && tj === common_1.FeatureType.HalogenAcceptor));
}
...
function testHalogenBond(structure, infoA, infoB, opts) {
    ...
    if (!isHalogenBond(typeA, typeB))
        return;
    const [don, acc] = typeA === common_1.FeatureType.HalogenDonor ? [infoA, infoB] : [infoB, infoA];
...
//
exports.HalogenDonorProvider = features_1.Features.Provider([common_1.FeatureType.HalogenDonor], addUnitHalogenDonors);
exports.HalogenAcceptorProvider = features_1.Features.Provider([common_1.FeatureType.HalogenAcceptor], addUnitHalogenAcceptors);
exports.HalogenBondsProvider = {
    ...
    createTester: (props) => {
        ...
        return {
            ...
            requiredFeatures: new Set([common_1.FeatureType.HalogenDonor, common_1.FeatureType.HalogenAcceptor]),
