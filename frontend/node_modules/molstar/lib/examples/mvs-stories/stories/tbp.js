...
import { isPrimitiveComponentExpressions } from '../../../extensions/mvs/tree/mvs/param-types';
...
const Superpositions = {
    '1cdw': [-0.4665815186, 0.6063873444, -0.6438913535, 0, -0.581544075, -0.7588303199, -0.2932286385, 0, -0.6664144171, 0.2376361381, 0.7066971703, 0, 135.0863694935, 105.5007997009, 153.6890178993, 1],
    '1vtl': [-0.4769460004, 0.7214347188, -0.5020502557, 0, -0.297882932, 0.4047204695, 0.8645617968, 0, 0.8269149119, 0.5619014932, 0.0218732801, 0, 65.6043682658, -3.7328402905, -16.8650755387, 1],
    '7enc': [0.8975055044, -0.4316347566, -0.0904174009, 0, 0.247274877, 0.3227849997, 0.9136000105, 0, -0.3651561375, -0.8423189899, 0.3964337454, 0, -189.7572972798, 304.0841220076, -411.5005782853, 1],
};
const Steps = [
    {
...
function transform(structure, id) {
    ...
    const translation = Mat4.getTranslation(Vec3.zero(), Superpositions[id]);
...
function select(structure, { color, opacity = 1.0, selector = 'polymer' }) {
...
function label(structure, { selector, text }) {
    structure.component({ selector })
        .label({ text });
}
function surface(structure, options) {
    const comp = structure.component({ selector: options.selector });
    const coloring = {
        custom: {
            molstar_color_theme_name: 'element-symbol',
            molstar_color_theme_params: { carbonColor: (options === null || options === void 0 ? void 0 : options.carbon_color) ? { name: 'uniform', params: { value: decodeColor(options === null || options === void 0 ? void 0 : options.carbon_color) } } : { name: 'element-symbol', params: {} } }
        }
    };
...
function bindingSite(structure, residues, options) {
    ...
    for (const [selector, label] of residues) {
        primitives.label({
            position: selector,
            text: label,
            label_color: (_b = options === null || options === void 0 ? void 0 : options.label_color) !== null && _b !== void 0 ? _b : color,
            label_size: (_c = options === null || options === void 0 ? void 0 : options.label_size) !== null && _c !== void 0 ? _c : 1.5
        });
    }
}
function pdbUrl(id) {
