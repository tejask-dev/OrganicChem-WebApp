...
exports.MaxAsa = exports.ANVILParams = void 0;
exports.computeANVIL = computeANVIL;
exports.calculate = calculate;
exports.isInMembranePlane = isInMembranePlane;
exports.isHydrophobic = isHydrophobic;
...
const shrake_rupley_1 = require("../../mol-model-props/computed/accessible-surface-area/shrake-rupley");
...
;
exports.ANVILParams = {
    numberOfSpherePoints: param_definition_1.ParamDefinition.Numeric(175, { min: 35, max: 700, step: 1 }, { description: 'Number of spheres/directions to test for membrane placement. Original value is 350.' }),
    stepSize: param_definition_1.ParamDefinition.Numeric(1, { min: 0.25, max: 4, step: 0.25 }, { description: 'Thickness of membrane slices that will be tested' }),
    minThickness: param_definition_1.ParamDefinition.Numeric(20, { min: 10, max: 30, step: 1 }, { description: 'Minimum membrane thickness used during refinement' }),
    maxThickness: param_definition_1.ParamDefinition.Numeric(40, { min: 30, max: 50, step: 1 }, { description: 'Maximum membrane thickness used during refinement' }),
    asaCutoff: param_definition_1.ParamDefinition.Numeric(40, { min: 10, max: 100, step: 1 }, { description: 'Relative ASA cutoff above which residues will be considered' }),
    adjust: param_definition_1.ParamDefinition.Numeric(14, { min: 0, max: 30, step: 1 }, { description: 'Minimum length of membrane-spanning regions (original values: 14 for alpha-helices and 5 for beta sheets). Set to 0 to not optimize membrane thickness.' }),
    tmdetDefinition: param_definition_1.ParamDefinition.Boolean(false, { description: `Use TMDET's classification of membrane-favoring amino acids. TMDET's classification shows better performance on porins and other beta-barrel structures.` })
};
 ...
 */
function computeANVIL(structure, props) {
    return mol_task_1.Task.create('Compute Membrane Orientation', async (runtime) => {
        return await calculate(runtime, structure, props);
    });
}
// avoiding namespace lookup improved performance in Chrome (Aug 2020)
const v3add = linear_algebra_1.Vec3.add;
const v3clone = linear_algebra_1.Vec3.clone;
const v3create = linear_algebra_1.Vec3.create;
...
const v3zero = linear_algebra_1.Vec3.zero;
const centroidHelper = new centroid_helper_1.CentroidHelper();
async function initialize(structure, props, accessibleSurfaceArea) {
    const l = structure_1.StructureElement.Location.create(structure);
    const { label_atom_id, label_comp_id, x, y, z } = structure_1.StructureProperties.atom;
    const asaCutoff = props.asaCutoff / 100;
    centroidHelper.reset();
    const offsets = new Array();
    const exposed = new Array();
    const hydrophobic = new Array();
    const definition = props.tmdetDefinition ? TMDET_DEFINITION : ANVIL_DEFINITION;
    function isPartOfEntity(l) {
        return !structure_1.Unit.isAtomic(l.unit) ? notAtomic() : l.unit.model.atomicHierarchy.residues.label_seq_id.valueKind(l.unit.residueIndex[l.element]) === 0;
    }
    const vec = v3zero();
    for (let i = 0, il = structure.units.length; i < il; ++i) {
        ...
        for (let j = 0, jl = elements.length; j < jl; ++j) {
            ...
            if ((0, util_1.getElementMoleculeType)(unit, eI) !== types_1.MoleculeType.Protein || !isPartOfEntity(l)) {
                continue;
            }
            ...
            if (!exports.MaxAsa[label_comp_id(l)]) {
                continue;
            }
            ...
            if (shrake_rupley_1.AccessibleSurfaceArea.getValue(l, accessibleSurfaceArea) / exports.MaxAsa[label_comp_id(l)] > asaCutoff) {
...
async function calculate(runtime, structure, params) {
    // can't get away with the default 92 points here
    const asaProps = { ...param_definition_1.ParamDefinition.getDefaultValues(accessible_surface_area_1.AccessibleSurfaceAreaParams), probeSize: 4.0, traceOnly: true, numberOfSpherePoints: 184 };
    const accessibleSurfaceArea = await shrake_rupley_1.AccessibleSurfaceArea.compute(structure, asaProps).runInContext(runtime);
...
async function findMembrane(runtime, message, ctx, spherePoints, initialStats) {
...
async function adjustThickness(runtime, message, ctx, membrane, initialHphobHphil) {
...
function membraneSegments(ctx, membrane) {
    ...
    const inMembrane = Object.create(null);
    const outMembrane = Object.create(null);
    const segments = [];
    let authAsymId;
    let lastAuthAsymId = null;
    let authSeqId;
    let lastAuthSeqId = units[0].model.atomicHierarchy.residues.auth_seq_id.value(units[0].chainIndex[0]) - 1;
    let startOffset = 0;
    let endOffset = 0;
...
function isInMembranePlane(testPoint, normalVector, planePoint1, planePoint2) {
    const d1 = -v3dot(normalVector, planePoint1);
    const d2 = -v3dot(normalVector, planePoint2);
...
function generateSpherePoints(ctx, numberOfSpherePoints) {
    ...
    for (let k = 1, kl = numberOfSpherePoints + 1; k < kl; k++) {
        ...
        const point = v3create(extent * Math.sin(phi) * Math.sin(theta) + centroid[0], extent * Math.cos(theta) + centroid[1], extent * Math.cos(phi) * Math.sin(theta) + centroid[2]);
        points[k - 1] = point;
        oldPhi = phi;
    }
    return points;
...
function isHydrophobic(definition, label_comp_id) {
