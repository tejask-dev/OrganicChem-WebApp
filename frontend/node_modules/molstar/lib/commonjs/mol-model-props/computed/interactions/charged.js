    ...
    while (residueIt.hasNext) {
        ...
        if (PositvelyCharged.includes(compId)) {
            ...
            for (let j = start; j < end; ++j) {
                if ((0, util_1.typeSymbol)(unit, j) === types_2.Elements.N && !types_1.ProteinBackboneAtoms.has((0, util_1.atomId)(unit, j))) {
                    builder.pushMember(x[elements[j]], y[elements[j]], z[elements[j]], j);
                }
            }
            builder.finishState(common_1.FeatureType.PositiveCharge, common_1.FeatureGroup.None);
        }
            ...
            for (let j = start; j < end; ++j) {
                ...
                if (group) {
                    ...
                    (0, util_1.eachBondedAtom)(structure, unit, j, (_, k) => {
                        if ((0, util_1.typeSymbol)(unit, k) === types_2.Elements.N) {
                            addedElements.add(k);
                            builder.pushMember(x[elements[k]], y[elements[k]], z[elements[k]], k);
                        }
                    });
                    builder.finishState(common_1.FeatureType.PositiveCharge, group);
                }
            }
            for (let j = start; j < end; ++j) {
                if (charge[j] > 0 && !addedElements.has(j)) {
                    builder.add(common_1.FeatureType.PositiveCharge, common_1.FeatureGroup.None, x[elements[j]], y[elements[j]], z[elements[j]], j);
                }
            }
        }
    }
...
function addUnitNegativeCharges(structure, unit, builder) {
    ...
    while (residueIt.hasNext) {
        ...
        if (NegativelyCharged.includes(compId)) {
            builder.startState();
            for (let j = start; j < end; ++j) {
                if ((0, util_1.typeSymbol)(unit, j) === types_2.Elements.O && !types_1.ProteinBackboneAtoms.has((0, util_1.atomId)(unit, j))) {
                    builder.pushMember(x[elements[j]], y[elements[j]], z[elements[j]], j);
                }
            }
            builder.finishState(common_1.FeatureType.NegativeCharge, common_1.FeatureGroup.None);
        }
            ...
            for (let j = start; j < end; ++j) {
                if ((0, functional_group_1.isPhosphate)(structure, unit, j)) {
                    builder.startState();
                    (0, util_1.eachBondedAtom)(structure, unit, j, (_, k) => {
                        if ((0, util_1.typeSymbol)(unit, k) === types_2.Elements.O) {
                            builder.pushMember(x[elements[k]], y[elements[k]], z[elements[k]], k);
                        }
                    });
                    builder.finishState(common_1.FeatureType.NegativeCharge, common_1.FeatureGroup.Phosphate);
                }
            }
        }
        else if (!types_1.PolymerNames.has(compId)) {
            for (let j = start; j < end; ++j) {
                ...
                if (group) {
                    ...
                    builder.finishState(common_1.FeatureType.NegativeCharge, group);
                }
            }
            for (let j = start; j < end; ++j) {
                if (charge[j] < 0 && !addedElements.has(j)) {
                    builder.add(common_1.FeatureType.NegativeCharge, common_1.FeatureGroup.None, x[elements[j]], y[elements[j]], z[elements[j]], j);
                }
            }
        }
    }
...
function addUnitAromaticRings(structure, unit, builder) {
    ...
    for (const ringIndex of unit.rings.aromaticRings) {
        ...
        builder.finishState(common_1.FeatureType.AromaticRing, common_1.FeatureGroup.None);
    }
}
function isIonic(ti, tj) {
    return ((ti === common_1.FeatureType.NegativeCharge && tj === common_1.FeatureType.PositiveCharge) ||
        (ti === common_1.FeatureType.PositiveCharge && tj === common_1.FeatureType.NegativeCharge));
}
function isPiStacking(ti, tj) {
    return ti === common_1.FeatureType.AromaticRing && tj === common_1.FeatureType.AromaticRing;
}
function isCationPi(ti, tj) {
    return ((ti === common_1.FeatureType.AromaticRing && tj === common_1.FeatureType.PositiveCharge) ||
        (ti === common_1.FeatureType.PositiveCharge && tj === common_1.FeatureType.AromaticRing));
}
...
function testCationPi(structure, infoA, infoB, distanceSq, opts) {
    ...
    if (isCationPi(typeA, typeB)) {
        const [infoR, infoC] = typeA === common_1.FeatureType.AromaticRing ? [infoA, infoB] : [infoB, infoA];
...
//
exports.NegativChargeProvider = features_1.Features.Provider([common_1.FeatureType.NegativeCharge], addUnitNegativeCharges);
exports.PositiveChargeProvider = features_1.Features.Provider([common_1.FeatureType.PositiveCharge], addUnitPositiveCharges);
exports.AromaticRingProvider = features_1.Features.Provider([common_1.FeatureType.AromaticRing], addUnitAromaticRings);
exports.IonicProvider = {
    ...
    createTester: (props) => {
        ...
        return {
            ...
            requiredFeatures: new Set([common_1.FeatureType.NegativeCharge, common_1.FeatureType.PositiveCharge]),
...
exports.PiStackingProvider = {
    ...
    createTester: (props) => {
        ...
        return {
            maxDistance: props.distanceMax,
            requiredFeatures: new Set([common_1.FeatureType.AromaticRing]),
...
exports.CationPiProvider = {
    ...
    createTester: (props) => {
        ...
        return {
            maxDistance: props.distanceMax,
            requiredFeatures: new Set([common_1.FeatureType.AromaticRing, common_1.FeatureType.PositiveCharge]),
