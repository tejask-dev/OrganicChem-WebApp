...
function build(data, boundary, cellSizeOrCount) {
    ...
    const expandedBox = box3d_1.Box3D.expand((0, box3d_1.Box3D)(), boundary.box, linear_algebra_1.Vec3.create(0.5, 0.5, 0.5));
    const { indices } = data;
    const S = box3d_1.Box3D.size((0, linear_algebra_1.Vec3)(), expandedBox);
    let delta, size;
    const elementCount = int_1.OrderedSet.size(indices);
        ...
        for (let iy = loY; iy <= hiY; iy++) {
            for (let iz = loZ; iz <= hiZ; iz++) {
                ...
                for (let i = offset; i < end; i++) {
                    const idx = int_1.OrderedSet.getAt(indices, bucketArray[i]);
                    const dx = px[idx] - x;
                    const dy = py[idx] - y;
                    const dz = pz[idx] - z;
...
function _insideOut(r) {
    ...
    for (let x = 0; x < n; ++x) {
        for (let y = 0; y < n; ++y) {
            for (let z = 0; z < n; ++z) {
                cells.push(linear_algebra_1.Vec3.create(x - r, y - r, z - r));
            }
        }
    }
    cells.sort((a, b) => linear_algebra_1.Vec3.squaredMagnitude(a) - linear_algebra_1.Vec3.squaredMagnitude(b));
    return cells.flat();
}
const insideOut = (0, memoize_1.memoize1)(_insideOut);
/**
...
const tmpDirVec = (0, linear_algebra_1.Vec3)();
...
const tmpArrG2 = [0.1];
const tmpArrG3 = [0.1];
const tmpHeapG = new fibonacci_heap_1.FibonacciHeap();
function queryNearest(ctx, result) {
    ...
    const indicesCount = int_1.OrderedSet.size(indices);
    common_1.Result.reset(result);
            ...
            if (!tmpSetG.has(gridId)) {
                ...
                if (bucketIdx !== 0) {
                    ...
                    for (let i = offset; i < end; i++) {
                        ...
                        const idx = int_1.OrderedSet.getAt(indices, bIdx);
                        const dx = px[idx] - x;
                        const dy = py[idx] - y;
                        const dz = pz[idx] - z;
