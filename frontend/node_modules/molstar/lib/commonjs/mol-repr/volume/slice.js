...
exports.SliceParams = {
    ...
    mode: param_definition_1.ParamDefinition.Select('grid', param_definition_1.ParamDefinition.arrayToOptions(['grid', 'frame', 'plane']), { description: 'Grid: slice through the volume along the grid axes in integer steps. Frame: slice through the volume along arbitrary axes in any step size. Plane: an arbitrary plane defined by point and normal.' }),
    ...
    plane: param_definition_1.ParamDefinition.Group({
        point: param_definition_1.ParamDefinition.Vec3(vec3_1.Vec3.create(0, 0, 0), {}, { description: 'Plane point' }),
        normal: param_definition_1.ParamDefinition.Vec3(vec3_1.Vec3.create(1, 0, 0), {}, { description: 'Plane normal' }),
...
async function createImage(ctx, volume, key, theme, props, image) {
    switch (props.mode) {
        ...
        default:
            (0, type_helpers_1.assertUnreachable)(props.mode);
    }
}
//
...
function getSampledImage(volume, theme, info, isoValue, trim, image) {
    ...
    const im = image_1.Image.create(imageTexture, corners, groupTexture, valueTexture, trim, isoLevel, image);
    im.setBoundingSphere(volume_1.Volume.getBoundingSphere(volume));
    return im;
}
...
async function createPlaneImage(ctx, volume, key, theme, props, image) {
    ...
    const m = mat4_1.Mat4.fromPlane((0, mat4_1.Mat4)(), normal, point);
    const gridToCartn = volume_1.Grid.getGridToCartesianTransform(volume.grid);
    const cartnToGrid = mat4_1.Mat4.invert((0, mat4_1.Mat4)(), gridToCartn);
    ...
    const scaleFactor = 1 / resolution;
    const s = vec3_1.Vec3.distance(vec3_1.Vec3.create(0, 0, 0), vec3_1.Vec3.create(a, b, c)) * Math.SQRT2;
    const size = vec3_1.Vec3.set((0, vec3_1.Vec3)(), s, s, s);
    mat4_1.Mat4.mul(m, m, mat4_1.Mat4.rotY90);
    mat4_1.Mat4.scale(m, m, size);
    const width = Math.floor(size[1] * scaleFactor);
    const height = Math.floor(size[0] * scaleFactor);
    const trim = {
        type: 3,
    ...
    return (0, representation_1.VolumeVisual)({
        ...
        setUpdateState: (state, volume, newProps, currentProps, newTheme, currentTheme) => {
            state.createGeometry = (newProps.dimension.name !== currentProps.dimension.name ||
                ...
                !vec3_1.Vec3.equals(newProps.plane.point, currentProps.plane.point) ||
                !vec3_1.Vec3.equals(newProps.plane.normal, currentProps.plane.normal) ||
