...
(function (Loci) {
    ...
    function fromExpression(structure, expression, queryContext) {
        ...
        if (typeof expression === 'function') {
            expr = expression(builder_1.MolScriptBuilder);
        }
        else {
            expr = expression;
        }
        const selection = (0, base_1.compile)(expr)(queryContext !== null && queryContext !== void 0 ? queryContext : new query_1.QueryContext(structure));
        return query_1.StructureSelection.toLociWithSourceUnits(selection);
    }
    Loci.fromExpression = fromExpression;
    function fromQuery(structure, query, queryContext) {
        const selection = query(queryContext !== null && queryContext !== void 0 ? queryContext : new query_1.QueryContext(structure));
        return query_1.StructureSelection.toLociWithSourceUnits(selection);
    }
    Loci.fromQuery = fromQuery;
    function fromSchema(structure, schema, queryContext) {
        return schema_1.Schema.toLoci(structure, schema, queryContext);
    }
    ...
    function forEachLocation(loci, f, location) {
        if (Loci.isEmpty(loci))
            return;
        const loc = location ? location : location_1.Location.create(loci.structure);
        ...
        for (const e of loci.elements) {
            ...
            for (let i = 0, _i = int_1.OrderedSet.size(indices); i < _i; i++) {
                loc.element = elements[int_1.OrderedSet.getAt(indices, i)];
                f(loc);
            }
        }
    }
    ...
    function extendToWholeResidues(loci, restrictToConformation) {
        ...
        for (const lociElement of loci.elements) {
            ...
            if (lociElement.unit.kind === unit_1.Unit.Kind.Atomic) {
    ...
    function getChainSegments(unit) {
        switch (unit.kind) {
            case unit_1.Unit.Kind.Atomic: return unit.model.atomicHierarchy.chainAtomSegments;
            case unit_1.Unit.Kind.Spheres: return unit.model.coarseHierarchy.spheres.chainElementSegments;
            case unit_1.Unit.Kind.Gaussians: return unit.model.coarseHierarchy.gaussians.chainElementSegments;
        }
    }
