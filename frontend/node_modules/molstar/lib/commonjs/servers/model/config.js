...
exports.ModelServerConfigTemplate = exports.ModelServerConfig = exports.mapSourceAndIdToFilename = exports.ModelServerFetchFormats = void 0;
...
exports.ModelServerFetchFormats = ['cif', 'bcif', 'cif.gz', 'bcif.gz'];
let mapSourceAndIdToFilename = () => {
    throw new Error('call setupConfig & validateConfigAndSetupSourceMap to initialize this function');
};
exports.mapSourceAndIdToFilename = mapSourceAndIdToFilename;
function addServerArgs(parser) {
    ...
    parser.add_argument('--sourceMap', {
        ...
        help: [
            ...
            `Supported formats: ${exports.ModelServerFetchFormats.join(', ')}`
        ].join('\n'),
    });
    parser.add_argument('--sourceMapUrl', {
        ...
        help: [
            ...
            `Supported formats: ${exports.ModelServerFetchFormats.join(', ')}. Supported protocols: http://, https://, gs://`
        ].join('\n'),
    });
...
exports.ModelServerConfig = { ...DefaultModelServerConfig };
exports.ModelServerConfigTemplate = {
...
delete exports.ModelServerConfigTemplate.customProperties;
...
function setConfig(config) {
    for (const k of (0, type_helpers_1.ObjectKeys)(exports.ModelServerConfig)) {
        if (config[k] !== void 0)
            exports.ModelServerConfig[k] = config[k];
    }
    if (config.sourceMapUrl) {
        if (!exports.ModelServerConfig.sourceMap)
            exports.ModelServerConfig.sourceMap = [];
        exports.ModelServerConfig.sourceMap.push(...config.sourceMapUrl);
    }
}
function validateConfigAndSetupSourceMap() {
    if (!exports.ModelServerConfig.sourceMap || exports.ModelServerConfig.sourceMap.length === 0) {
        throw new Error(`Please provide 'sourceMap' configuration. See [-h] for available options.`);
    }
    exports.mapSourceAndIdToFilename = new Function('source', 'id', [
        'switch (source.toLowerCase()) {',
        ...exports.ModelServerConfig.sourceMap.map(([source, path, format]) => `case '${source.toLowerCase()}': return [\`${path}\`, '${format}'];`),
        '}',
    ].join('\n'));
}
function parseConfigArguments() {
    const parser = new argparse.ArgumentParser({
        add_help: true,
        description: `ModelServer ${version_1.VERSION}, (c) 2018-2020, Mol* contributors`
    });
...
function configureServer() {
    ...
    if (!!config.cfgTemplate) {
        console.log(JSON.stringify(exports.ModelServerConfigTemplate, null, 2));
        process.exit(0);
    }
    try {
        setConfig(config); // sets the config for global use
        if (config.cfg) {
            const cfg = JSON.parse(fs.readFileSync(config.cfg, 'utf8'));
            setConfig(cfg);
        }
        if (!!config.printCfg) {
            console.log(JSON.stringify(exports.ModelServerConfig, null, 2));
            process.exit(0);
        }
