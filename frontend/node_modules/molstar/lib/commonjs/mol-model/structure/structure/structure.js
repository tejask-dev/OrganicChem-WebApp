...
function getEntityIndices(structure) {
    ...
    for (const unit of units) {
        const prop = unit.kind === unit_1.Unit.Kind.Atomic ? properties_1.StructureProperties.entity.key : properties_1.StructureProperties.coarse.entityKey;
        l.unit = unit;
...
(function (Structure) {
    ...
    function ofModel(model, props = {}) {
        ...
        for (let c = 0; c < chains.count; c++) {
            ...
            builder.addUnit(unit_1.Unit.Kind.Atomic, model, operator, elements, traits);
        }
        const cs = model.coarseHierarchy;
        if (cs.isDefined) {
            if (cs.spheres.count > 0) {
                addCoarseUnits(builder, model, model.coarseHierarchy.spheres, unit_1.Unit.Kind.Spheres);
            }
            if (cs.gaussians.count > 0) {
                addCoarseUnits(builder, model, model.coarseHierarchy.gaussians, unit_1.Unit.Kind.Gaussians);
            }
        }
    ...
    function eachAtomicHierarchyElement(structure, { chain, residue, atom }) {
        const l = element_1.StructureElement.Location.create(structure);
        for (const unit of structure.units) {
            if (unit.kind !== unit_1.Unit.Kind.Atomic)
                continue;
            l.unit = unit;
            const { elements } = unit;
            const chainsIt = int_1.Segmentation.transientSegments(unit.model.atomicHierarchy.chainAtomSegments, elements);
