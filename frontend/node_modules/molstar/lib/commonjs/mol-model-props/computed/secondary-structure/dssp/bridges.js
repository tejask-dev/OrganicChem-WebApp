    ...
    for (let k = 0, kl = proteinInfo.residueIndices.length; k < kl; ++k) {
        for (let t = offset[k], _t = offset[k + 1]; t < _t; t++) {
            ...
            if (i !== j && hbonds.getDirectedEdgeIndex(j, i + 1) !== -1) {
                flags[i] |= common_1.DSSPType.Flag.B;
                flags[j] |= common_1.DSSPType.Flag.B;
                // TODO move to constructor, actually omit object all together
                bridges[bridges.length] = new common_1.Bridge(i, j, common_1.BridgeType.PARALLEL);
            }
            ...
            if (i !== j && hbonds.getDirectedEdgeIndex(j - 1, i) !== -1) {
                flags[i] |= common_1.DSSPType.Flag.B;
                flags[j] |= common_1.DSSPType.Flag.B;
                bridges[bridges.length] = new common_1.Bridge(j, i, common_1.BridgeType.PARALLEL);
            }
            ...
            if (i !== j && hbonds.getDirectedEdgeIndex(j, i) !== -1) {
                flags[i] |= common_1.DSSPType.Flag.B;
                flags[j] |= common_1.DSSPType.Flag.B;
                bridges[bridges.length] = new common_1.Bridge(j, i, common_1.BridgeType.ANTI_PARALLEL);
            }
