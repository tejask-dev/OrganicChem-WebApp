    ...
    async open(file) {
        try {
            ...
            if (fn.endsWith('json') || fn.endsWith('molj')) {
                ...
                const snapshot = JSON.parse(string_like_1.StringLike.toString(data));
                ...
                const snapshot = await this.plugin.runTask((0, data_source_1.readFromFile)(stateFile, 'json'));
                if (data['assets.json']) {
                    const file = new File([data['assets.json']], 'assets.json');
                    const json = await this.plugin.runTask((0, data_source_1.readFromFile)(file, 'json'));
