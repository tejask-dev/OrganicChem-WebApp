...
function Plane3D() {
    return Plane3D.create(vec3_1.Vec3.create(1, 0, 0), 0);
}
(function (Plane3D) {
    function create(normal, constant) { return { normal, constant }; }
    Plane3D.create = create;
    function copy(out, p) {
        vec3_1.Vec3.copy(out.normal, p.normal);
    ...
    function normalize(out, p) {
        ...
        const inverseNormalLength = 1.0 / vec3_1.Vec3.magnitude(p.normal);
        vec3_1.Vec3.scale(out.normal, p.normal, inverseNormalLength);
        out.constant = p.constant * inverseNormalLength;
        return out;
    }
    Plane3D.normalize = normalize;
    function negate(out, p) {
        vec3_1.Vec3.negate(out.normal, p.normal);
        out.constant = -p.constant;
        return out;
    }
    Plane3D.negate = negate;
    function toArray(p, out, offset) {
        vec3_1.Vec3.toArray(p.normal, out, offset);
        out[offset + 3] = p.constant;
        return out;
    }
    Plane3D.toArray = toArray;
    function fromArray(out, array, offset) {
        vec3_1.Vec3.fromArray(out.normal, array, offset);
    ...
    function fromNormalAndCoplanarPoint(out, normal, point) {
        ...
        out.constant = -vec3_1.Vec3.dot(out.normal, point);
        return out;
    }
    Plane3D.fromNormalAndCoplanarPoint = fromNormalAndCoplanarPoint;
    function fromCoplanarPoints(out, a, b, c) {
        const normal = vec3_1.Vec3.triangleNormal((0, vec3_1.Vec3)(), a, b, c);
        fromNormalAndCoplanarPoint(out, normal, a);
        return out;
    }
    Plane3D.fromCoplanarPoints = fromCoplanarPoints;
    const unnormTmpV = (0, vec3_1.Vec3)();
    function setUnnormalized(out, nx, ny, nz, constant) {
        vec3_1.Vec3.set(unnormTmpV, nx, ny, nz);
        const inverseNormalLength = 1.0 / vec3_1.Vec3.magnitude(unnormTmpV);
        vec3_1.Vec3.scale(out.normal, unnormTmpV, inverseNormalLength);
        out.constant = constant * inverseNormalLength;
        return out;
    }
    Plane3D.setUnnormalized = setUnnormalized;
    function distanceToPoint(plane, point) {
        return vec3_1.Vec3.dot(plane.normal, point) + plane.constant;
    }
    ...
    function projectPoint(out, plane, point) {
        return vec3_1.Vec3.scaleAndAdd(out, point, plane.normal, -distanceToPoint(plane, point));
    }
    Plane3D.projectPoint = projectPoint;
})(Plane3D || (exports.Plane3D = Plane3D = {}));
