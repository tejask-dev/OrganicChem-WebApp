...
const bonds_1 = require("../../../../mol-model/structure/model/properties/atomic/bonds");
...
(function (StructConn) {
    ...
    function getEntriesFromStructConn(struct_conn, model) {
        ...
        const _p = (row, ps) => {
            if (ps.label_asym_id.valueKind(row) !== db_1.Column.ValueKinds.Present)
                return void 0;
            ...
            const resId = (ps.auth_seq_id.valueKind(row) === db_1.Column.ValueKind.Present) ?
        ...
        for (let i = 0; i < struct_conn._rowCount; i++) {
            ...
            let flags = types_1.BondType.Flag.None;
            let order = 1;
            switch (orderType) {
                case 'sing':
                    order = 1;
            ...
            switch (type) {
                case 'covale':
                    flags = types_1.BondType.Flag.Covalent;
                    break;
                case 'disulf':
                    flags = types_1.BondType.Flag.Covalent | types_1.BondType.Flag.Disulfide;
                    break;
                case 'hydrog':
                    flags = types_1.BondType.Flag.HydrogenBond;
                    break;
                case 'metalc':
                    flags = types_1.BondType.Flag.MetallicCoordination;
                    break;
            }
            entries.push({
...
})(StructConn || (exports.StructConn = StructConn = {}));
