...
const symbols = [
    ...
    C(symbol_table_1.MolScriptSymbolTable.structureQuery.type.bondFlags, (ctx, xs) => {
        let ret = types_1.BondType.Flag.None;
        if (typeof xs.length === 'number') {
            for (let i = 0, _i = xs.length; i < _i; i++)
    ...
    C(symbol_table_1.MolScriptSymbolTable.structureQuery.type.secondaryStructureFlags, (ctx, xs) => {
        let ret = types_1.SecondaryStructureType.Flag.None;
        if (typeof xs.length === 'number') {
            for (let i = 0, _i = xs.length; i < _i; i++)
    ...
    D(symbol_table_1.MolScriptSymbolTable.structureQuery.atomProperty.ihm.hasSeqId, function structureQuery_atomProperty_ihm_hasSeqId(ctx, xs) {
        const current = ctx.element;
        const seqId = (xs && xs[0] && xs[0](ctx));
        if (current.unit.kind === structure_1.Unit.Kind.Atomic) {
            return seqId === structure_1.StructureProperties.residue.label_seq_id(current);
        }
        return seqId >= structure_1.StructureProperties.coarse.seq_id_begin(current) && seqId <= structure_1.StructureProperties.coarse.seq_id_end(current);
    }),
    D(symbol_table_1.MolScriptSymbolTable.structureQuery.atomProperty.ihm.overlapsSeqIdRange, function structureQuery_atomProperty_ihm_hasSeqId(ctx, xs) {
        var _a, _b;
        const current = ctx.element;
        ...
        if (current.unit.kind === structure_1.Unit.Kind.Atomic) {
            const value = structure_1.StructureProperties.residue.label_seq_id(current);
            return value >= beg && value <= end;
        }
        const a = structure_1.StructureProperties.coarse.seq_id_begin(current);
        const b = structure_1.StructureProperties.coarse.seq_id_end(current);
...
function bondFlag(current, f) {
    return current | (types_1.BondType.isName(f) ? types_1.BondType.fromName(f) : types_1.BondType.Flag.None);
}
function secondaryStructureFlag(current, f) {
    switch (f.toLowerCase()) {
        case 'helix': return current | types_1.SecondaryStructureType.Flag.Helix;
        case 'alpha': return current | types_1.SecondaryStructureType.Flag.Helix | types_1.SecondaryStructureType.Flag.HelixAlpha;
        case 'pi': return current | types_1.SecondaryStructureType.Flag.Helix | types_1.SecondaryStructureType.Flag.HelixPi;
        case '310': return current | types_1.SecondaryStructureType.Flag.Helix | types_1.SecondaryStructureType.Flag.Helix3Ten;
        case 'beta': return current | types_1.SecondaryStructureType.Flag.Beta;
        case 'strand': return current | types_1.SecondaryStructureType.Flag.Beta | types_1.SecondaryStructureType.Flag.BetaStrand;
        case 'sheet': return current | types_1.SecondaryStructureType.Flag.Beta | types_1.SecondaryStructureType.Flag.BetaSheet;
        case 'turn': return current | types_1.SecondaryStructureType.Flag.Turn;
        case 'bend': return current | types_1.SecondaryStructureType.Flag.Bend;
        case 'coil': return current | types_1.SecondaryStructureType.Flag.NA;
