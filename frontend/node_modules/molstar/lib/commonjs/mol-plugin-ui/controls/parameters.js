...
const colorGradientBanded = (0, memoize_1.memoize1)((colors) => {
    ...
    if (hasOffsets) {
        ...
        off.sort((a, b) => a[1] - b[1]);
        styles.push(`${color_1.Color.toStyle(off[0][0])} ${(100 * off[0][1]).toFixed(2)}%`);
        for (let i = 0, il = off.length - 1; i < il; ++i) {
            const [c0, o0] = off[i];
            const [c1, o1] = off[i + 1];
            const o = o0 + (o1 - o0) / 2;
            styles.push(`${color_1.Color.toStyle(c0)} ${(100 * o).toFixed(2)}%`, `${color_1.Color.toStyle(c1)} ${(100 * o).toFixed(2)}%`);
        }
        styles.push(`${color_1.Color.toStyle(off[off.length - 1][0])} ${(100 * off[off.length - 1][1]).toFixed(2)}%`);
    }
    else {
