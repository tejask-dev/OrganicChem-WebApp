...
function schemaItemToExpression(item) {
    ...
    const { macromolecular, ihm, core } = builder_1.MolScriptBuilder.struct.atomProperty;
    const propTests = {};
    ...
    if (isDefined(item.atom_index))
        atomTests.push(eq([builder_1.MolScriptBuilder.struct.atomProperty.core.sourceIndex(), item.atom_index]));
    ...
    if (isDefined(item.type_symbol))
        atomTests.push(eq([builder_1.MolScriptBuilder.struct.atomProperty.core.elementSymbol(), item.type_symbol.toUpperCase()]));
...
function toLoci(structure, schema, queryContext) {
    ...
    return loci_1.Loci.fromExpression(structure, expr, queryContext);
}
function toBundle(structure, schema, queryContext) {
    const loci = toLoci(structure, schema, queryContext);
    return bundle_1.Bundle.fromLoci(loci);
}
exports.Schema = {
    forEachItem,
    toExpression,
